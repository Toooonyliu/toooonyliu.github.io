...
test('benefits require an explicit recognized status valid for the complete journey',()=>{
 const accounts=sampleAccounts(),o=offersFor(q)[0];assert.equal(benefitsFor(o,q,accounts).tierModeled,true);assert.equal(benefitsFor(o,q,[{...accounts[0],currentTier:undefined}]).tierModeled,false);assert.equal(benefitsFor(o,q,[{...accounts[0],source:'manual'}]).tierModeled,true);assert.equal(benefitsFor(o,{...q,returnDate:'2027-05-01'},accounts).tierModeled,false);assert.equal(benefitsFor(offersFor({...q,cabin:'Business'})[0],q,[]).perks.length,0);
});
